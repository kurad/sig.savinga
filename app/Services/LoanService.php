...
class LoanService
{
    ...
    public function listLoans(?int $userId = null, ?string $status = null, int $perPage = 15)
    {
        ...
        $p->getCollection()->transform(function (Loan $loan) {
            ...
            $interest = round((float) ($loan->interest_amount ?? 0), 2);
            ...
            $outstanding = [
                ...
                'interest_amount'  => round((float) $loan->outstandingInterest(), 2),
            ];
    ...
    public function repayWithAutoSplit(
        ...
        return DB::transaction(function () use ($loanId, $amount, $paidDate, $recordedBy) {
            ...
            // ✅ Compute outstanding ONCE (rounded)
            $outstanding = round((float) $loan->outstandingBalance(), 2);
            ...
            if ($outstanding <= 0) {
                $contrib = $this->contributionService->record(
                    ...
                    period: $paid->format('Y-m') // or UI-selected period if you add it
                );
            ...
            // ✅ Internal repay (NO strict outstanding check here)
    ...
    /**
     * ✅ Internal helper: assumes Loan is already locked inside the current transaction
     * and amount was already capped by repayWithAutoSplit.
     */
    ...
    public function topUpAsNewLoan(
        ...
        return DB::transaction(function () use (
            ...
            // ✅ create schedule (same logic as disburse())
            $base = round($totalPayable / $duration, 2);
            $sumFirst = round($base * ($duration - 1), 2);
            $lastAmt  = round($totalPayable - $sumFirst, 2);

            for ($i = 1; $i <= $duration; $i++) {
                $instDue = Carbon::parse($topUpLoan->issued_date, $tz)
                    ->addMonthsNoOverflow($i)
                    ->toDateString();
    ...
    public function eligibilityPreview(
        ...
        string $interestBasis = 'per_year',
        ?int $interestTermMonths = null,
        ?string $repaymentMode = 'once',     // once | installment
        ?int $durationMonths = 1             // used if installment
    ): array {
        if (!in_array($viewer->role, ['admin', 'treasurer'], true) && $viewer->id !== $memberId) {
            throw new \Exception('Forbidden');
        }
        ...
        // Rule 2: months contributed
        $minMonths = (int)($rules->min_contribution_months ?? 3);
        $months = (int) $this->contributionService->contributedMonthsCount($memberId);

        // Rule 3: max allowed (3x savings base incl opening balance)
        $savingBase = (float) $this->contributionService->savingsBaseForLoanLimit($memberId);

        $multiple = (float)($rules->loan_limit_value ?? 3);
        $maxAllowed = round($savingBase * $multiple, 2);
        ...
        $interest = $this->computeInterest(
            ...
            rate: $interestRate,
            basis: $interestBasis,
            durationMonths: ($repaymentMode === 'installment') ? $durationMonths : 1,
            termMonths: $interestTermMonths
        );
        ...
        if ($repaymentMode === 'installment') {
            $monthlyInstallment = round($totalPayable / $durationMonths, 2);
        }

        return [
            // your existing eligibility outputs
            'savings_base' => round($savingBase, 2),
    ...
    public function repayPreview(int $loanId, User $viewer): array
    {
        ...
        // Safety: never negative (in case of rounding or data issues)
